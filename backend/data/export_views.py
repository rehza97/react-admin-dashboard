...
class RevenueCollectionExportView(BaseExportView):
    ...
    def get(self, request):
        ...
        if month:
            collection_query = collection_query.filter(invoice_date__month=month)
        if dot:
            collection_query = collection_query.filter(organization__icontains=dot)
        
        # Prepare export data
        headers = ['Invoice ID', 'Date', 'DOT', 'Customer', 'Revenue Amount', 'Invoiced Amount', 'Collection Amount']
        export_data = []
        
        # Add revenue data
        for rev in revenue_query:
            # Try to find matching collection record
            collection = collection_query.filter(invoice_number=rev.invoice_number).first()
            ...
            export_data.append([
                rev.invoice_number,
                rev.invoice_date.strftime('%Y-%m-%d') if rev.invoice_date else '',
                rev.organization,
                rev.customer,
    ...
    def get(self, request):
        ...
        for item in query:
            export_data.append([
                ...
                item.creation_date.strftime('%Y-%m-%d') if item.creation_date else '',
                item.state,
                item.offer_name,
                item.telecom_type,
        ...
        if dot and dot.strip():
            ...
            try:
                if dot.isdigit():
                    ...
                    query = query.filter(Q(dot_code__icontains=dot) | Q(dot_code=dot))
            except:
