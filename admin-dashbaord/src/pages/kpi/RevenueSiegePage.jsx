...
const RevenueSiegePage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await kpiService.getSiegeRevenueKPIs(filters);
      setData(response);
      ...
      console.error("Error fetching Siège revenue data:", err);
      setError("Failed to load Siège revenue data. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <PageLayout
      title="Siège Revenue Analysis"
      subtitle="Monitor and analyze Siège revenue performance"
    >
      {/* Filters */}
      <Box sx={{ p: 3, mb: 2 }}>
...
export default RevenueSiegePage;
