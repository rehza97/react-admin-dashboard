...
const CollectionsDOTCorporatePage = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await kpiService.getDOTCorporateCollectionKPIs(filters);
      setData(response);
      ...
      console.error("Error fetching DOT Corporate collection data:", err);
      setError(
        "Failed to load DOT Corporate collection data. Please try again later."
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <PageLayout
      title="DOT Corporate Collections Analysis"
      subtitle="Monitor and analyze DOT Corporate collections performance"
    >
      {/* Filters */}
      <Box sx={{ p: 3, mb: 2 }}>
          ...
          <Box sx={{ p: 3 }}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} md={6}>
                <Paper elevation={0} sx={{ p: 3, height: "400px" }}>
                  <Typography variant="h6" gutterBottom>
                    Monthly Collections vs. Invoiced Trends
                  </Typography>
                  <ResponsiveContainer width="100%" height="90%">
                    <LineChart
...
export default CollectionsDOTCorporatePage;
